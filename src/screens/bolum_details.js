...
class Bolum_details extends React.Component{
  ...
  uni_manager=new BaseManager()
    ...
    componentDidMount(){
     this.uni_manager.get_table_bolum_details(this.props.get_wanted_bolum_name).then((val)=>this.setState({data:val,data_two:val}))
     setTimeout(()=>this.setState({indicator:false}),200)

    }
    search_engine(val){
      this.search_manager.searching_detail_bolum(val,this.state.data).then((dat) => this.setState({data_two:dat}))
    }
            ...
            {(!this.state.indicator) ? 
              <SafeAreaView>
                <View style={child}>
                  ...
                  <Search place_text={"Aradığınız üniversitenin ismini girin"} search_data={(val)=>this.search_engine(val)}/>
                </View>
                <FlatList
                ...
                />
                <Direction_btn icon_name={"arrow-left"} handler_screen={()=>{navigation.navigate("Bolums"),Keyboard.dismiss()}} style_parent={{position:"absolute",bottom:10,right:10}} />
                </SafeAreaView>
                :
                <SafeAreaView style={main}>
                  <View style={child}> 
...
const mapStateToProps = state => {
    const {bolum} = state
    return {
        get_wanted_bolum_name:bolum.wanted_bolum
    }
  };
